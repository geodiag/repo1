...
import {
  ...
  ChevronRight, Info, Map, Landmark, TreePine,
} from "lucide-react";

// ═══════════════════════════════════════════════════════════════
//  MOCK DATA — remplacez par la réponse réelle de votre backend
// ═══════════════════════════════════════════════════════════════

const buildingData = {
  ban: {
    adresse:      "23 Rue du Maréchal Foch",
    code_postal:  "69003",
    ville:        "Lyon",
    departement:  "Rhône (69)",
    gps:          { lat: 45.748, lng: 4.857 },
    altitude:     173,
  },
  bdnb: {
    id:                "bdnb-bg-X8AY-QVG3-VCW6",
    type_bien:         "Appartement",
    annee_construction: 1965,
    nb_logements:      12,
    surface_shon:      850,
    hauteur_batiment:  14.5,
    usage:             "Résidentiel collectif",
  },
  ademe: {
    etiquette_dpe:    "E",
    conso_energie:    311,   // kWh/m²/an
    ges:              67,    // kgCO2/m²/an
    etiquette_ges:    "D",
    cout_chauffage:   "1 800 – 2 460 €/an",
    type_chauffage:   "Gaz collectif",
    type_ecs:         "Gaz individuel",
    type_isolation:   "Isolation partielle",
    type_toiture:     "Toit terrasse",
  ...
  cadastre: {
    ...
    section:         "AB",
    numero:          "0147",
    surface_parcelle: 420,   // m²
    adresse_parcelle: "23 RUE DU MARECHAL FOCH",
    contenance:       420,
  },
  gpu: {
    zone_plu:            "UA",
    libelle_zone:        "Zone urbaine mixte à dominante d'habitat",
  ...
  georisques: {
    ...
    ensa_bruit:              true,
    ensa_classe:             "Cl. 2",
    distance_basias:         320,    // mètres
    nb_basias_500m:          2,
    antennes_5g_proximite:   true,
    nb_antennes_5g:          3,
    inondation_ppr:          false,
    catnat_count:            3,
    icpe_count:              1,
    ret_gonflement:          "Moyen",
  },
};
...
/** Badge compact (type de bien, année…) */
function Badge({ children, color = "gray" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-semibold border ${colors[color]}`}>
      {children}
    </span>
  );
}
...
function DataRow({ label, value, sub, bold = false, className = "" }) {
  return (
    <div className={`flex items-start justify-between py-1 gap-2 ${className}`}>
      ...
      <span className={`text-[11px] text-right ${bold ? "font-semibold text-gray-800" : "text-gray-700"}`}>
        {value}
...
/** Barre de progression fine avec couleur conditionnelle */
...
/** Étiquette DPE (7 barres A→G) */
function DpeScale({ classeEnergie, classeGes, conso, ges }) {
  const classes = ["A", "B", "C", "D", "E", "F", "G"];
  const energyColors = {
    A: { bar: "#319834", text: "#1e5e20" },
    B: { bar: "#33c240", text: "#1e7a28" },
    C: { bar: "#cbe516", text: "#6e7a0b" },
    D: { bar: "#f7e614", text: "#7a7005" },
    E: { bar: "#f0ae13", text: "#7a5407" },
    F: { bar: "#e7731a", text: "#7a3808" },
    G: { bar: "#dd1e26", text: "#7a0f0f" },
  };
  const gesColors = {
    A: "#dfc4f7", B: "#c9a2e6", C: "#b580d5",
    D: "#a05ec4", E: "#8b3cb3", F: "#7618a1", G: "#5f0090",
  };

  const idxE = classes.indexOf(classeEnergie);
  const idxG = classes.indexOf(classeGes);
    ...
    <div className="flex gap-4 items-start">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="space-y-[3px]">
          {classes.map((cls, i) => {
            ...
            const c = energyColors[cls];
            const widths = [32, 40, 48, 57, 70, 82, 92];
            return (
              <div key={cls} className="flex items-center gap-1.5">
                <div
                  className="flex items-center justify-start pl-2 rounded-sm transition-all"
                  style={{
                ...
                >
                  <span
                    ...
                    style={{ color: "#fff", fontSize: isActive ? "11px" : "8px" }}
                  >
                    {cls}
                  </span>
                </div>
        ...
        <div className="space-y-[3px]">
          {classes.map((cls, i) => {
            ...
            return (
              <div key={cls} className="flex items-center gap-1.5">
                <div
                  className="flex items-center justify-start pl-2 rounded-sm"
                  style={{
                ...
                >
                  <span className="font-black leading-none text-white" style={{ fontSize: isActive ? "11px" : "8px" }}>
                    {cls}
                  </span>
                </div>
                {isActive && (
                  <span className="text-[10px] font-bold text-purple-700">{ges} kg</span>
                )}
              </div>
            );
          })}
      ...
      <div className="flex flex-col items-center justify-center gap-3 pl-2">
        <div className="text-center">
          <div
            className="w-12 h-12 rounded-lg flex items-center justify-center font-black text-2xl text-white shadow"
            style={{ backgroundColor: energyColors[classeEnergie]?.bar ?? "#888" }}
          >
            {classeEnergie}
          </div>
        ...
        <div className="text-center">
          <div
            className="w-12 h-12 rounded-lg flex items-center justify-center font-black text-2xl text-white shadow"
            style={{ backgroundColor: gesColors[classeGes] ?? "#888" }}
          >
            {classeGes}
          </div>
...
/** Ligne risque avec icône check / alert / cross */
function RiskRow({ label, value, status }) {
  const icon =
    status === "ok"   ? <CheckCircle size={12} className="text-emerald-500 flex-shrink-0" /> :
    status === "warn" ? <AlertTriangle size={12} className="text-orange-500 flex-shrink-0" /> :
                        <XCircle size={12} className="text-red-500 flex-shrink-0" />;
  const textColor =
    status === "ok"   ? "text-emerald-700" :
    status === "warn" ? "text-orange-700"  :
                        "text-red-700";
  return (
...
/** Pill de score */
function ScorePill({ value, max, label, color }) {
  const colors = {
  ...
  return (
    <div className={`inline-flex flex-col items-center justify-center px-3 py-1.5 rounded-md border text-center ${colors[color]}`}>
      <span className="text-lg font-black leading-none">{value}{max ? `/${max}` : ""}</span>
    ...
    <div className="min-h-screen bg-gray-50 font-sans" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-bold text-gray-700 tracking-tight">DIAGX</span>
        ...
        <div className="flex items-center gap-2">
          <Badge color="blue"><Info size={10} /> Données Open Data</Badge>
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-4 space-y-3">
        ...
        <Card>
          <div className="px-5 py-4 flex items-start justify-between gap-4">
            <div className="flex items-start gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[13px] text-gray-500 font-medium mt-0.5">
                  {ban.code_postal} {ban.ville} — {ban.departement}
                </p>
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <span className="text-[10px] text-gray-400 font-mono">
                    {ban.gps.lat}°N, {ban.gps.lng}°E · Alt. {ban.altitude} m
                  </span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
              ...
              <ScorePill value={ademe.etiquette_dpe} label="DPE" color={
                ["A","B","C"].includes(ademe.etiquette_dpe) ? "green" :
                ["D","E"].includes(ademe.etiquette_dpe)     ? "orange" : "red"
              } />
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          {/* ── PERFORMANCE ÉNERGÉTIQUE (8 colonnes) ─────────── */}
          <div className="col-span-8 space-y-3">
            ...
            <Card>
              ...
              <div className="p-4">
                <DpeScale
                  classeEnergie={ademe.etiquette_dpe}
                  classeGes={ademe.etiquette_ges}
                  conso={ademe.conso_energie}
                  ges={ademe.ges}
                />
            ...
            <Card>
              ...
              <div className="px-4 py-3 grid grid-cols-3 gap-x-6 gap-y-0.5 divide-x divide-gray-100">
          ...
          {/* ── IDENTITÉ PARCELLAIRE (4 colonnes) ──────────────── */}
          <div className="col-span-4 space-y-3">
            ...
            <Card>
              ...
              <div className="px-4 py-3 space-y-0.5">
                ...
                <RiskRow
                  ...
                  value={gpu.droit_preemption ? gpu.type_dpu : "Non"}
                  status={gpu.droit_preemption ? "warn" : "ok"}
                />
                <RiskRow
                  label="Secteur sauvegardé"
                  value={gpu.secteur_sauvegarde ? "Oui" : "Non"}
                  status={gpu.secteur_sauvegarde ? "warn" : "ok"}
                />
                <RiskRow
                  label="Plan de risque"
                  value={gpu.plan_risque ? "Oui" : "Non"}
                  status={gpu.plan_risque ? "warn" : "ok"}
                />
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="px-4 py-3">
                <div className="bg-gray-100 rounded-md h-24 flex items-center justify-center border border-gray-200 mb-2 relative overflow-hidden">
                  <div className="absolute inset-0 opacity-10"
                    style={{
                      backgroundImage: "repeating-linear-gradient(0deg,#94a3b8 0,#94a3b8 1px,transparent 1px,transparent 20px),repeating-linear-gradient(90deg,#94a3b8 0,#94a3b8 1px,transparent 1px,transparent 20px)"
                    }}
                  />
                  <div className="text-center z-10">
                    <MapPin size={18} className="text-blue-500 mx-auto" />
                    <span className="text-[9px] font-semibold text-gray-500 block mt-1">Vue cartographique</span>
                  </div>
                </div>
        ...
        {/* ═══ ÉTAT DES RISQUES (Full Width) ═══ */}
        <Card>
          <CardHeader
            icon={ShieldAlert}
            title="État des Risques et Pollutions (ERP)"
            color="text-red-500"
            source="Source : Géorisques / GASPAR"
          />
          <div className="grid grid-cols-3 divide-x divide-gray-100">
            ...
            <div className="p-4 space-y-3">
              ...
              <ProgressBar
                value={georisques.sismicite} max={5}
                level={sismiLevel}
                label="Sismicité"
                sublabel={`Zone ${georisques.sismicite}/5`}
              />
              <ProgressBar
                value={georisques.argiles === "Fort" ? 3 : georisques.argiles === "Moyen" ? 2 : 1} max={3}
                level={argileLevel}
                label="Retrait-gonflement argiles"
                sublabel={georisques.argiles}
              />
              <ProgressBar
                value={georisques.radon} max={3}
                level={radonLevel}
                label="Potentiel Radon"
                sublabel={`Zone ${georisques.radon}/3`}
              />
              <ProgressBar
                value={georisques.ret_gonflement === "Fort" ? 3 : georisques.ret_gonflement === "Moyen" ? 2 : 1} max={3}
                level={retGonfLevel}
                label="Ret.-gonfl. sols argileux"
                sublabel={georisques.ret_gonflement}
              />

              {/* CatNat */}
              <div className="flex items-center justify-between pt-1 border-t border-gray-100">
                <span className="text-[11px] text-gray-500">Arrêtés CatNat</span>
                <Badge color={georisques.catnat_count > 0 ? "orange" : "green"}>
                  {georisques.catnat_count} arrêté{georisques.catnat_count > 1 ? "s" : ""}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-gray-500">Inondation PPR</span>
                <Badge color={georisques.inondation_ppr ? "red" : "green"}>
                  {georisques.inondation_ppr ? "Classé" : "Non classé"}
                </Badge>
              </div>
              <p className="text-[10px] text-gray-400 pt-1">Source : Géorisques / GASPAR</p>
            </div>

            {/* ── Bloc 2 : Nuisances Biologiques & Sonores ── */}
            <div className="p-4">
              <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400 mb-3">Nuisances & Santé</p>
              <div className="space-y-0.5">
                <RiskRow
                  label="Termites"
                  value={georisques.termites ? "Zone infestée" : "Non concerné"}
                  status={georisques.termites ? "danger" : "ok"}
                />
                <RiskRow
                  label="Mérule"
                  value={georisques.merule ? "Zone à risque" : "Non concerné"}
                  status={georisques.merule ? "danger" : "ok"}
                />
                <RiskRow
                  label="Bruit aérien (ENSA)"
                  value={georisques.ensa_bruit ? `Classé ${georisques.ensa_classe}` : "Non concerné"}
                  status={georisques.ensa_bruit ? "warn" : "ok"}
                />
              </div>
              ...
              <div className="space-y-0.5">
                <RiskRow
                  label="Sites ICPE à proximité"
                  value={`${georisques.icpe_count} site${georisques.icpe_count > 1 ? "s" : ""}`}
                  status={georisques.icpe_count > 0 ? "warn" : "ok"}
                />
              </div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-1.5">
                {[
                  ...
                  <div
                    key={label}
                    className={`rounded-md p-2 flex flex-col items-center gap-1 border ${
                      active
            ...
            {/* ── Bloc 3 : Proximité & Environnement ── */}
            <div className="p-4">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-md p-3 mb-2">
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <Badge color={georisques.distance_basias < 200 ? "red" : georisques.distance_basias < 500 ? "orange" : "green"}>
                    {georisques.distance_basias} m
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-gray-500">Nb sites dans 500 m</span>
                  <Badge color={georisques.nb_basias_500m > 0 ? "orange" : "green"}>
                    {georisques.nb_basias_500m} site{georisques.nb_basias_500m > 1 ? "s" : ""}
                  </Badge>
                </div>
                {/* Barre de distance */}
                <div className="mt-2 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, (georisques.distance_basias / 1000) * 100)}%` }}
                  />
                </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-md p-3 mb-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge color={georisques.antennes_5g_proximite ? "blue" : "green"}>
                    {georisques.antennes_5g_proximite ? `${georisques.nb_antennes_5g} antennes` : "Aucune"}
                  </Badge>
                </div>
              </div>
              ...
              <div className="space-y-0.5 mt-3">
                <div className="flex items-center justify-between py-1 border-b border-gray-100">
                  <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wide">Indicateur</span>
                  <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wide">Statut</span>
                </div>
                {[
                  { label: "Sismicité",   val: `Zone ${georisques.sismicite}`, color: sismiLevel  === "high" ? "red" : sismiLevel  === "medium" ? "orange" : "green" },
                  { label: "Radon",       val: `Classe ${georisques.radon}`,   color: radonLevel  === "high" ? "red" : radonLevel  === "medium" ? "orange" : "green" },
                  { label: "Argiles",     val: georisques.argiles,             color: argileLevel === "high" ? "red" : argileLevel === "medium" ? "orange" : "green" },
                  { label: "Termites",    val: georisques.termites ? "Oui" : "Non", color: georisques.termites ? "red" : "green" },
                  { label: "ICPE",        val: `${georisques.icpe_count} site`, color: georisques.icpe_count > 0 ? "orange" : "green" },
                ].map(({ label, val, color }) => (
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-400 pb-2">
          <span>Données issues de l'Open Data gouvernemental français — BAN · BDNB · ADEME · IGN · Géorisques · ANFR</span>
